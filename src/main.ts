type Route = '/functions' | '/excel' | '/batch' | '/notes' | '/updates' | '/settings';
...
(async () => {
  ...
  // Завантажуємо і застосовуємо тему при запуску
  const savedTheme = await window.api?.getSetting?.('theme', 'system');
  if (savedTheme) {
...
byId<HTMLButtonElement>('btn-process-order')?.addEventListener('click', async () => {
  try {
    ...
    // 3.5. Отримання шляху до Excel файлу (якщо режим Розпорядження)
    let excelPath: string | undefined = undefined;
    if (flags.isOrder) {
      const excelPathInput = byId<HTMLInputElement>('excel-path');
      excelPath = excelPathInput?.value || undefined;
      if (excelPath) {
        log(`📊 Excel файл: ${excelPath}`);
      } else {
        log('⚠️ Excel файл не вибрано - буде використано тільки пошук "розпорядженні"');
      }
    }
    
    // 4. Виклик IPC для обробки з Excel
    const payload = {
      wordBuf,
      outputPath,
      excelPath,
...
// Ініціалізація теми для випадків, коли settings сторінка ще не завантажена
(async () => {
  // Затримуємо ініціалізацію, щоб дати час DOM завантажитись
  setTimeout(async () => {
    const sel = byId<HTMLSelectElement>('theme-select');
    if (!sel) return;
    const val = await window.api?.getSetting?.('theme', 'system');
    if (val) sel.value = val;
    // Обробник тепер додається в setupExtendedSettingsAutoSave
  }, 500);
})();
...
class UpdateManager {
  ...
  private async loadCurrentVersion() {
    ...
    if (versionEl) versionEl.textContent = '1.2.7';
  }
...
function setupExtendedSettingsAutoSave() {
  ...
  // Додаємо обробник для теми
  const themeSelect = byId<HTMLSelectElement>('theme-select');
  if (themeSelect) {
    themeSelect.addEventListener('change', async () => {
      await window.api?.setSetting?.('theme', themeSelect.value);
      applyTheme(themeSelect.value); // Застосовуємо тему одразу
...
async function loadAppInfo() {
  try {
    ...
    if (versionEl) versionEl.textContent = version || 'Невідома';
...
function importSettings() {
  ...
  input.addEventListener('change', async (e) => {
    ...
    try {
      ...
      await loadSettings(); // Оновлюємо основні налаштування
...
// Обробники подій для кнопок налаштувань
byId('export-settings')?.addEventListener('click', exportSettings);
byId('import-settings')?.addEventListener('click', importSettings);
byId('reset-settings')?.addEventListener('click', resetSettings);

// Навігація - завантаження розширених налаштувань при переході на settings
window.addEventListener('hashchange', () => {
  if (location.hash === '#/settings') {
    // Затримка для завантаження DOM
    setTimeout(() => {
      loadAllSettings();
      setupExtendedSettingsAutoSave();
      
      // Додаємо анімації для settings секцій
...
window.addEventListener('load', () => {
  // Завжди ініціалізуємо тему, навіть якщо не на settings сторінці
  setTimeout(async () => {
    const sel = byId<HTMLSelectElement>('theme-select');
    if (sel) {
      const val = await window.api?.getSetting?.('theme', 'system');
      ...
      // Застосовуємо завантажену тему
      applyTheme(val);
      
      // Додаємо обробник для зміни теми (якщо ще не доданий)
      if (!sel.dataset.initialized) {
        sel.addEventListener('change', async () => {
          await window.api?.setSetting?.('theme', sel.value);
          applyTheme(sel.value); // Застосовуємо тему одразу
        });
...
// Імпорт типів для зведення
type Mode = 'БЗ' | 'ЗС' | 'Обидва';
...
class ExcelProcessor {
  ...
  private setupEventListeners() {
    ...
    selectFolderBtn?.addEventListener('click', async () => {
      try {
        ...
        this.log(`❌ Помилка вибору папки: ${error}`, 'error');
      }
    });
    ...
    selectDestinationBtn?.addEventListener('click', async () => {
      try {
        // Тут би був виклик до API для вибору Excel файлу
        ...
        if (mockResult?.filePath) {
          ...
          this.updateProcessButton();
          this.log(`💾 Обрано файл призначення: ${mockResult.filePath}`);
          this.saveDestinationSettings();
        }
      } catch (error) {
        this.log(`❌ Помилка вибору файлу призначення: ${error}`, 'error');
      }
    });
  ...
  private async startSummarizationProcessing() {
    if (this.isProcessing) return;
    
    // Валідація вхідних даних
    if (!this.inputFolder || !this.destinationFile) {
      this.log('❌ Не всі обов\'язкові поля заповнені', 'error');
      return;
    }
    ...
    this.log(`🚀 Початок зведення Excel файлів. Режим: ${mode}`);
    ...
    try {
      ...
      this.log(`📁 Папка джерела: ${payload.srcFolder}`);
      this.log(`💾 Файл призначення: ${payload.dstPath}`);
      this.log(`📊 Режим обробки: ${payload.mode}`);

      // Mock обробки (в реальному застосунку тут би був виклик до нового API)
      await this.mockSummarizationProcess(payload);
      
    } catch (error) {
      this.log(`❌ Помилка зведення: ${error}`, 'error');
    } finally {
      this.stopProcessing();
    }
  }

  private async mockSummarizationProcess(payload: StartProcessPayload) {
    // Mock процес зведення для демонстрації
    this.log('� Сканування папки та завантаження конфігурації...');
    ...
    this.log(`🔧 Активні опції: ${activeOptions.join(', ')}`);
    ...
    for (let i = 0; i < modes.length; i++) {
      ...
      this.log(`� Режим ${i + 1}/${modes.length}: ${currentMode}`);
      ...
      this.log(`📋 Обробка ${rules.length} правил для режиму ${currentMode}`);
      
      for (let j = 0; j < rules.length; j++) {
        if (!this.isProcessing) break;
        
        const rule = rules[j];
        this.log(`📄 Правило "${rule}": пошук файлу...`);
        ...
        if (Math.random() > 0.2) { // 80% успішності
          ...
          totalFiles++;
          totalRows += rows;
          this.log(`✅ Правило "${rule}": знайдено файл, скопійовано ${rows} рядків`);
        } else {
          this.log(`⚠️ Правило "${rule}": файл не знайдено`, 'error');
        }
      }
      
      this.log(`✅ Режим "${currentMode}" завершено`);
    }
    ...
    this.log(`🎉 Зведення завершено успішно!`);
    this.log(`📈 Підсумок: файлів - ${totalFiles}, рядків - ${totalRows}`);
    this.log(`💾 Результат збережено: ${payload.dstPath}`);
  ...
  private stopProcessing() {
    ...
    this.updateProcessButton();
    this.log('⏹️ Обробку зупинено користувачем');
  }

  private updateProgress(percent: number, status: string, detail: string) {
  ...
  private async saveLogs() {
    try {
      ...
      if (!logsContent.trim()) {
        this.log('⚠️ Немає логів для збереження');
        return;
      }
      ...
      // Тут би був виклик до electron API для збереження файлу
      // const result = await window.api?.saveFile?.(filename, logsContent);
      
      this.log(`💾 Логи збережено: ${filename}`);
    } catch (error) {
      this.log(`❌ Помилка збереження логів: ${error}`, 'error');
    }
  }
  ...
  private async loadSavedSettings() {
    try {
      ...
      this.log('📁 Збережені налаштування завантажено');
    } catch (error) {
      console.warn('Не вдалося завантажити збережені налаштування:', error);
    }
  }

  private async saveDestinationSettings() {
    try {
      const rememberDestinationCheckbox = byId<HTMLInputElement>('excel-remember-destination');
  ...
  private async saveProcessingSettings() {
    try {
      const sliceCheck = byId<HTMLInputElement>('excel-slice-check')?.checked || false;
      const mismatches = byId<HTMLInputElement>('excel-mismatches')?.checked || false;
      const sanitizer = byId<HTMLInputElement>('excel-sanitizer')?.checked || false;
      ...
      this.log(`💾 Налаштування збережено: Slice_Check=${sliceCheck}, Mismatches=${mismatches}, Sanitizer=${sanitizer}`);
    } catch (error) {
      console.warn('Не вдалося зберегти налаштування обробки:', error);
    }
  }

  private async saveSummarizationMode() {
    try {
      const selectedMode = document.querySelector('input[name="excel-mode"]:checked') as HTMLInputElement;
      ...
      this.log(`💾 Режим зведення збережено: ${mode}`);
    } catch (error) {
...
// Ініціалізуємо Excel процесор
let excelProcessor: ExcelProcessor;

// Додаємо ініціалізацію Excel процесора до існуючого обробника load
