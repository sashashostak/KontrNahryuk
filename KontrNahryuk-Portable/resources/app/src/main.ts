...
class ExcelProcessor {
  ...
  private outputFilename: string = 'результат_обробки.xlsx';
  ...
  private setupEventListeners() {
    ...
    const selectOutputBtn = byId('excel-select-output');
    const outputLocationField = byId<HTMLInputElement>('excel-output-location');
    
    selectOutputBtn?.addEventListener('click', async () => {
      try {
        const result = await window.api?.selectFolder?.();
        if (result?.filePath) {
          this.outputLocation = result.filePath;
          outputLocationField!.value = result.filePath;
          this.updateProcessButton();
          this.log(`💾 Обране місце збереження: ${result.filePath}`);
        }
      } catch (error) {
        this.log(`❌ Помилка вибору місця збереження: ${error}`, 'error');
      }
    });

    // Зміна назви файлу
    const outputFilenameField = byId<HTMLInputElement>('excel-output-filename');
    outputFilenameField?.addEventListener('input', () => {
      this.outputFilename = outputFilenameField.value || 'результат_обробки.xlsx';
      this.updateProcessButton();
    });
  ...
  private async scanForExcelFiles() {
    try {
      // TODO: В майбутньому використаємо includeSubfolders для рекурсивного пошуку
      // const includeSubfolders = byId<HTMLInputElement>('excel-include-subfolders')?.checked || false;
  ...
  private updateProcessButton() {
    ...
    const canProcess = this.inputFolder && this.outputLocation && this.outputFilename && this.foundFiles.length > 0 && !this.isProcessing;
  ...
  private async startProcessing() {
    ...
    this.isProcessing = true;
    this.log(`🚀 Початок обробки ${this.foundFiles.length} файлів...`);
    ...
    try {
      // Емуляція обробки файлів
      for (let i = 0; i < this.foundFiles.length; i++) {
        if (!this.isProcessing) break;
        
        const file = this.foundFiles[i];
        const progress = Math.round(((i + 1) / this.foundFiles.length) * 100);
        
        this.updateProgress(progress, `Обробка ${file}`, `Файл ${i + 1}/${this.foundFiles.length}`);
        this.log(`📄 Обробляється: ${file}`);
        
        // Емуляція часу обробки
        await new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 2000));
        
        if (this.isProcessing) {
          this.log(`✅ Завершено: ${file}`);
        }
      }
      
      if (this.isProcessing) {
        const outputPath = `${this.outputLocation}/${this.outputFilename}`;
        this.log(`🎉 Обробка завершена! Результат збережено: ${outputPath}`);
        this.updateProgress(100, 'Завершено', `Оброблено ${this.foundFiles.length} файлів`);
      }
      
    } catch (error) {
      this.log(`❌ Помилка обробки: ${error}`, 'error');
    } finally {
      this.stopProcessing();
    }
  }
